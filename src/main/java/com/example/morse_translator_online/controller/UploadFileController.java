...
public class UploadFileController {
    ...
    public ResponseEntity<?> uploadFile(@RequestParam("file")MultipartFile file){
        try {
            ...
            FileWriter fileWriter = new FileWriter("D:\\JAVA_projects\\morse_translator_online\\src\\main\\resources\\output\\output.txt");
            PrintWriter printWriter = new PrintWriter(fileWriter);
            printWriter.print(morseTranslatorService.morseConvert(Paths.get("D:\\JAVA_projects\\morse_translator_online\\src\\main\\resources\\input\\input.txt"),1)); //1:en->morse   else    morse->en
            printWriter.close();
        } catch (IOException e) {
